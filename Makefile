...
CPP_OBJS := $(patsubst $(APP_DIR)/%.cpp, $(OBJ_DIR)/%.o, $(filter %.cpp, $(APPS)))
...
simulator:
	g++ -O3 -o doodle_emulator doodle_emulator.cpp `sdl2-config --cflags --libs`
...
$(CPP_OBJS): $(OBJ_DIR)/%.o: $(APP_DIR)/%.cpp $(RUNTIME_DIR)/librv32iemu.a
	mkdir -p $(OBJ_DIR)
	$(CXX) $(CFLAGS) -o $@ -c $<
