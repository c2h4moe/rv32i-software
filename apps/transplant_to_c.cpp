...
//每调用一次为所有时间变量加一，即时间增加一帧
void playerclass_plusplus(playerclass* player)
{
	player->t++;
...
//bottom_x是判断玩家是否与地面接触的底部x点坐标
int bottom_x(playerclass player)
{
	return player.pos_x + 52;
}
//bottom_y是判断玩家是否与地面接触的底部y点坐标
int bottom_y(playerclass player)
{
	return player.pos_y + 116;
} 
//contact_x是判断玩家是否与物品接触的x点坐标
int contact_x(playerclass player)
{
	return player.pos_x + 65;
}
//contact_y是判断玩家是否与物品接触的y点坐标
int contact_y(playerclass player)
{
	return player.pos_y + 60;
}
//初始设置设置玩家的新x，y坐标，以及人物状态
void playerclass_set(playerclass* player, int nx, int ny, PLAYER_STATUS ns)
{
	player->pos_x = nx;
	player->pos_y = ny;
	player->status = ns;
	player->jump_from_y = pos_y;
	return;
}
//每帧根据状态调整高度
