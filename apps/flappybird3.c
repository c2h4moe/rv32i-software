...
int barrier_num;
int start_flag;
int g_ThroughX[3], g_ThroughY[3]; // 通道基准中心坐标
int g_Result = 0;				  // 1---游戏结束     0---游戏未结束
int g_Score;					  // 分数成绩
int Book[3][61] = {0};			  // 标记地图上的位置是否有障碍物
...
void StartUp()
{
	...
	g_MapHight = 60;
	g_MapWidth = 80;
	g_BridX = g_MapWidth / 3;
	g_BridY = g_MapHight / 2;
	index = 0;
	g_ThroughX[0] = g_MapWidth / 3 * 2;
	g_ThroughY[0] = g_MapHight / 2;
	g_ThroughX[1] = g_MapWidth;
	g_ThroughY[1] = g_MapHight / 2;
	start_flag = 0;
	barrier_num = 2;
...
void StartMap()
{
	...
	{
		for (int i = 1; i <= g_ThroughY[j] - 3; i++)
		{
		...
		for (int i = g_ThroughY[j] + 4; i < g_MapHight - 1; i++)
		{
			DrawPoint(g_ThroughX[j], i, yellow);
			DrawPoint(g_ThroughX[j] + 1, i, yellow);
			DrawPoint(g_ThroughX[j] + 2, i, black);
...
{
	g_BridY++;
	DrawPoint(g_BridX, g_BridY, cyan);
	...
	if (g_ThroughX[index] < g_MapWidth / 3)
	{
		g_ThroughY[(index + 2) % barrier_num] = 12 % g_MapHight;
		g_ThroughX[(index + 2) % barrier_num] = g_MapWidth;
...
void UpdateInPut()
{
	...
	if (ch == ' ' || ch == 'w')
	{
		...
		DrawPoint(g_BridX, g_BridY - 2, cyan);
		g_BridY -= 2;
	}
}
